...
import { normalizeProblem, selectFollowUps, type FollowUpQuestion, type NormalizedProblem } from "@/lib/analyze";
...
export default function AdvancedAnalyzePage() {
  ...
  const onConfirm = async (skipToDecision: boolean) => {
    ...
    if (r) {
      ...
      if (skipToDecision || remaining === 0) setStep("Decision");
  ...
  return (
    <div className="min-h-screen bg-paper pb-20">
      <div className="mx-auto max-w-3xl px-5 pt-28 sm:px-8 lg:pt-36">
        ...
        <div className="mb-6">
          <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-accent-deep">Describe an Operational Problem</p>
          <h1 className="mt-2 text-title font-semibold tracking-tight text-ink">Tell Compass about your problem.</h1>
          <Link href="/analyze" className="mt-2 inline-block text-[12px] font-medium text-accent-deep underline underline-offset-2 transition-colors hover:text-ink">
            ← Prefer five quick choices? Use the guided flow
          </Link>
        </div>
