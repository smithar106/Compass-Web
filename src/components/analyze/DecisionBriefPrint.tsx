...
export function DecisionBriefPrint({ recs, meta, summary, status, library, onClose }: DecisionBriefPrintProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-y-auto bg-[#101826]/70 p-4 sm:p-8" role="dialog" aria-modal="true" aria-label="Executive Decision Brief preview" onClick={onClose}>
      <div className="mx-auto max-w-4xl" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[#dfe5ec] bg-white px-5 py-3">
          ...
          <div className="flex items-center gap-2">
            <button type="button" onClick={handleDownload} className="inline-flex items-center gap-2 rounded-lg bg-ink px-5 py-2.5 text-[13px] font-semibold text-paper transition-colors hover:bg-ink2">Download PDF</button>
            <button type="button" onClick={onClose} className="rounded-lg border border-[#dfe5ec] bg-surface px-4 py-2.5 text-[13px] font-semibold text-ink transition-colors hover:border-ink/40">Close</button>
          </div>
        </div>
        ...
        <div id="compass-brief-print" ref={contentRef} className="mx-auto bg-white px-8 py-7 shadow-[0_25px_50px_rgba(0,0,0,0.25)] sm:px-10" style={{ width: 816, maxWidth: "100%", color: "#1c1a17", fontFamily: "ui-sans-serif, system-ui, -apple-system, Helvetica, Arial, sans-serif", lineHeight: 1.5 }}>
          ...
          <div className="mt-4 grid grid-cols-2 gap-px overflow-hidden rounded border border-[#e6e2db] bg-[#e6e2db] sm:grid-cols-4">
            <PrintMeta label="Prepared for" value="Executive Leadership" /><PrintMeta label="Decision" value={top.category ? top.category.replace(/_/g, " ") : "Operational intervention"} /><PrintMeta label="Status" value={badge.text} highlight /><PrintMeta label="Date" value={today} />
          </div>
          ...
          <PrintSection number="3" title="What could prevent success?" tone="amber">
          ...
          <PrintSection number="4" title="What happens after approval?" tone="teal">
          ...
          <div className="mt-5 border-t border-[#e6e2db] pt-2.5">
            ...
            <p className="mt-1 text-[10.5px] leading-[1.5] text-[#9c968a]">{decisionNotes()}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <section className="mt-6">
      <div className="mb-2.5 flex items-center gap-3">
        <span aria-hidden="true" className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full font-mono text-[12px] font-bold text-white" style={{ backgroundColor: c.accent }}>{number}</span>
...
function Sub({ label, tone }: { label: string; tone: BriefTone }) {
  return <p className="mt-4 mb-1.5 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.12em] first:mt-0 print-avoid-break" style={{ color: BRIEF_COLORS[tone].ink }}><span aria-hidden="true" className="h-1 w-3 rounded-full" style={{ backgroundColor: BRIEF_COLORS[tone].accent }} />{label}</p>;
}

function PrintMeta({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return <div className={cn("bg-white px-4 py-3", highlight && "bg-[#e9f6ee]")}><p className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#6c685f]">{label}</p><p className={cn("mt-0.5 text-[13px] font-semibold text-[#1c1a17]", highlight && "text-[#14663a]")}>{value}</p></div>;
}
